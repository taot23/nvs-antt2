...
interface VirtualizedSalesCardsProps {
  data: Sale[];
  isLoading: boolean;
  error: Error | null;
...
// Componente de card individual para uso dentro da lista virtualizada
const SaleCard: React.FC<{
  ...
  // Otimização: adicionar "will-change" para melhorar o desempenho durante a rolagem
  const cardStyle = {
    ...style,
    height: "auto",
    paddingBottom: "8px",
  ...
  return (
    <div 
      style={cardStyle} 
      className="virtual-sale-card"
      data-virtual-index={sale.id}
    >
      <Card 
        className="relative overflow-hidden border mb-2"
...
const VirtualizedSalesCards: React.FC<VirtualizedSalesCardsProps> = ({
  data,
  isLoading,
  error,
  onViewDetails,
  ...
  // Altura aproximada de cada card (ajuste conforme necessário)
  ...
  // Componente de item que será usado pela lista virtualizada
  ...
  return (
    <div style={{ height: 'calc(100vh - 230px)', width: '100%' }} className="virtualized-container">
      <AutoSizer>
        {({ height, width }) => (
          <List
            ref={listRef}
...
export default VirtualizedSalesCards;