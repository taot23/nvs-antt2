...
interface NativeSalesScrollerProps {
  data: Sale[];
  isLoading: boolean;
  error: Error | null;
...
const SaleCard: React.FC<{
  ...
  return (
    <Card 
      className="native-sale-card border mb-4 relative overflow-hidden"
...
// Componente principal com solução nativa para scrolling
const NativeSalesScroller: React.FC<NativeSalesScrollerProps> = ({
  data,
  isLoading,
  error,
  onViewDetails,
  ...
  // Configuração nativa para solução de scroll
  useEffect(() => {
    if (scrollerRef.current) {
      ...
      // Usa setAttribute para propriedades não padrão como WebkitOverflowScrolling
      scrollEl.setAttribute('style', 'overflow: auto; height: calc(100vh - 230px); max-height: calc(100vh - 230px); overscroll-behavior: contain; -webkit-overflow-scrolling: touch;');
  ...
  if (isLoading) {
    return (
      <div className="space-y-4" ref={scrollerRef}>
        {[...Array(3)].map((_, i) => (
  ...
  // Componente para exibir em caso de erro
  if (error) {
    return (
      <div className="text-center py-4 px-2 bg-red-50 rounded-md text-red-500 border border-red-200" ref={scrollerRef}>
  ...
  if (data.length === 0) {
    return (
      <div className="text-center py-4 bg-muted/20 rounded-md" ref={scrollerRef}>
  ...
  return (
    <div 
      ref={scrollerRef} 
      className="native-sales-scroller"
      style={{
...
export default NativeSalesScroller;